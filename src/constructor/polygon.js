...
/**
 * Generates Polygon geometry buffers
    ...
    for (const ring of geometry) {
        ...
        for (let j: number = 0; j < ring.length; j++) {
            for (let l: number = 0; l < ring[j].length - 1; l++) {
                vertices[0].push((ring[j][l].x - tileExtend), (ring[j][l].y - tileExtend));
                if (j > 0) {
...
function flatPolygon(vertices: number[][], data: IWorkerData, style: ILayerStyle): IWorkerData {
    ...
    for (const tri of tris) {
        ...
        data.vecData.push(
            vertices[0][tri * 2],
            style.min_height_tmp + data.layerOrder,
            vertices[0][tri * 2 + 1],
        );
        data.colData.push(
